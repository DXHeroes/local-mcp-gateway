...
export function resolveApiUrl(
  ...
  metaEnv: ImportMetaEnv | undefined = (import.meta as ImportMetaWithEnv).env
): string {
  const runtimeApiUrl = runtimeConfig.API_URL?.trim();
  if (runtimeApiUrl) {
  ...
  const buildTimeApiUrl = metaEnv?.VITE_API_URL?.trim();
  return buildTimeApiUrl ? trimTrailingSlash(buildTimeApiUrl) : '';
}
